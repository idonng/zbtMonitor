...
@
-moz-document url-prefix          () { /*针对 Firefox 的 hack 代码*/ fieldset
	{
	display: table-cell;
}
...
<body>
	...
	<div id="header">
		<h1>
			<a href="dashboard.html">舆情监控平台</a>
		</h1>
	</div>
	<!--close-Header-part-->
		...
		<ul class="nav">
			<li class="dropdown" id="profile-messages"><a title="" href="#"
				...
				class="dropdown-toggle"><i class="icon icon-user"></i> <span
					class="text">欢迎 ${user.userName}</span><b class="caret"></b> </a>
	...
	<div id="sidebar">
		<ul>
			...
			<li class="active"><a onclick="selectAllTemp();"> <span>模板设置</span> </a>
			</li>
		</ul>
	</div>
	<!--sidebar-menu-->
							...
							<c:forEach var="templist" varStatus="xh"
								...
								<tr id="del">
									...
									<td><a onclick="isDefault('${templist.tempId}');"><c:choose>
												<c:when
													test="${templist.tempId eq sessionScope.defaltId}">
		...
		function showTemp(id) {
			$.post("${ctx}/tempById.do", {
				...
				if (data.result == "ok") {
					...
					$("#Body").val(data.Body);
					if (data.Defalt == 0) {
						$("input[name='Default']").eq(1).attr("checked",
								"checked");
					}else{
						$("input[name='Default']").eq(1).attr("disabled",true);
					}
